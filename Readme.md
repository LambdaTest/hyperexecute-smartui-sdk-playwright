...
# How to run Playwright tests through SmartUI SDK on HyperExecute — TestMu AI (Formerly LambdaTest)
...
Download or Clone the code sample for the Playwright from the TestMu AI GitHub repository to run the tests on the HyperExecute.

```bash
git clone https://github.com/LambdaTest/hyperexecute-smartui-sdk-playwright
...
env:
  PROJECT_TOKEN: "YPUR_PROJECT_TOKEN" #Enter your project token here
...
post:
  - cat hyp-smartui-sdk-playwright.yaml
...
```bash
./hyperexecute --user user1 --key 12345 --config hyp-smartui-sdk-playwright.yaml
```
